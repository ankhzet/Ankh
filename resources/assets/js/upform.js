...
var
	upform = (typeof upform != 'undefined') ? upform : (new function () {
		var BTN_CLOSE = 0x000001;
		var div = null;
		this.buildCtls = function (controls) {
			if (!controls || !controls.length)
				return '';

			var pattern = '<a class="button {%class}" {%href} data-action="{%action}" onclick="{%onclick}">{%caption}</a>';
			var c = [], p = null;

			for (var i in controls) {
				switch (p = controls[i]) {
				case BTN_CLOSE:
					p = {"caption": "Закрыть", "action": "javascript:upform.close()", "class": ""};
					break;
				default:
				...
				c.push((typeof p != "string") ? patternize(pattern, p) : p);
			}
		...
		this.init = function (params) {
			...
			var controls = params.controls || [BTN_CLOSE];
...
function show_message(title, contents) {
	upform.init({
		title: title
